...
export function TaggedSearchPage({
  options,
  previewAssetVersion,
  onOpenPlayer,
  onOpenVLC,
  ...
  const visibleSubCategories = useMemo(() => {
    ...
    return options.categories.filter(
      (item) =>
        item.kind === "sub" &&
        item.parent_id !== null &&
        mainCategoryIds.includes(item.parent_id),
    );
  ...
  async function load() {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to search tagged content",
      );
      setData({
        items: [],
        total: 0,
    ...
    <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="surface-card h-fit p-5 xl:sticky xl:top-6">
        ...
        <div className="mt-5 grid gap-4">
          ...
          <FieldBlock label="Sort Order">
            <Select
              value={sortDir}
              onChange={(event) => updateSort(event.target.value as "asc" | "desc")}
            >
          ...
          <FilterGroup
            ...
            selected={companyIds}
            onToggle={(value) =>
              toggleNumber("search_companies", companyIds, value)
            }
          />
          ...
          <FilterGroup
            ...
            onToggle={(value) =>
              toggleNumber("search_main_cats", mainCategoryIds, value)
            }
          />
          ...
          {mainCategoryIds.length > 0 ? (
            <FilterGroup
              ...
              items={visibleSubCategories.map((item) => ({
                id: item.id,
                name: item.name,
              }))}
              selected={subCategoryIds}
              onToggle={(value) =>
                toggleNumber("search_sub_cats", subCategoryIds, value)
              }
            />
          ) : null}
      ...
      <section className="surface-card min-w-0 p-5">
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <Button
            onClick={() =>
              setPage(Math.min(page + 1, Math.max(data.total_pages, 1)))
            }
...
function FilterGroup<T extends number | string>(props: {
  title: string;
